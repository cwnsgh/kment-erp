...
import { getManagedClientDetail, deleteManagedClients } from "@/app/actions/managed-client";
...
export default function ManagedClientDetailPage() {
  ...
  const [managedClient, setManagedClient] = useState<ManagedClientDetail | null>(null);
  ...
  const loadData = async () => {
    ...
    try {
      ...
      if (result.success && result.managedClient && result.client) {
        ...
        console.log(
          "Brand names:",
          result.client.sites?.map((s) => ({ brandName: s.brandName, hasValue: !!s.brandName })),
        );
      ...
      <div className="white_box">
        <div className={styles.boxInner}>
          <div className="table_group">
            ...
            <div className="table_item">
              ...
              <ul className="table_row">
                <li className="row_group">
                  ...
                  <div className="table_data">{clientData.address || clientData.addressDetail ? `${clientData.address || ""} ${clientData.addressDetail || ""}`.trim() : "-"}</div>
                </li>
              </ul>
              ...
              <div className="table_wrap">
                ...
                <ul className="table_row">
                  ...
                  <li className="row_group">
                    ...
                    <div className="table_data">
                      <span className={getStatusClass(managedClient.paymentStatus)}>{formatPaymentStatus(managedClient.paymentStatus)}</span>
                    </div>
                  </li>
                </ul>
                ...
                <ul className="table_row">
                  <li className="row_group">
                    ...
                    <div className="table_data">{managedClient.startDate && managedClient.endDate ? `${formatDate(managedClient.startDate)} - ${formatDate(managedClient.endDate)}` : "-"}</div>
                  </li>
                  <li className="row_group">
                    <div className="table_head">진행상황</div>
                    <div className="table_data">
                      <span className={getStatusClass(managedClient.status)}>{getStatusLabel(managedClient.status)}</span>
                    </div>
                  </li>
                </ul>
              </div>
            ...
            {/* 담당자 정보 */}
            <div className="table_item">
              <h2 className="table_title" onClick={() => setIsContactsOpen(!isContactsOpen)} style={{ cursor: "pointer" }}>
                담당자 정보
                <img src="/images/arrow_icon.svg" alt="" width={16} height={16} className={`table_toggle ${isContactsOpen ? styles.rotated : ""}`} />
              </h2>
              {isContactsOpen && (
                <div className="table_wrap">
            ...
            <div className="table_item table_item2">
              <h2 className="table_title" onClick={() => setIsSitesOpen(!isSitesOpen)} style={{ cursor: "pointer" }}>
                사이트 정보
                <img src="/images/arrow_icon.svg" alt="" width={16} height={16} className={`table_toggle ${isSitesOpen ? styles.rotated : ""}`} />
              </h2>
              {isSitesOpen && (
                <div className="table_wrap">
            ...
            {/* 비고 */}
            <div className="table_item">
              <h2 className="table_title" onClick={() => setIsNoteOpen(!isNoteOpen)} style={{ cursor: "pointer" }}>
                비고
                <img src="/images/arrow_icon.svg" alt="" width={16} height={16} className={`table_toggle ${isNoteOpen ? styles.rotated : ""}`} />
              </h2>
              {isNoteOpen && (
                <div className="table_wrap">
