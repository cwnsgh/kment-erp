...
import { getWorkRequestsByClientIdForEmployee, updateWorkRequestStatus, getWorkRequestDetailForEmployee, getAllEmployees, type WorkRequest } from "@/app/actions/work-request";
...
export default function WorkRequestList({ clientId, clientName, currentEmployeeId, initialWorkRequests = [], initialTotalCount = 0 }: WorkRequestListProps) {
  ...
  const handleExcelDownload = () => {
    ...
    downloadExcel(`/api/operations/work-requests/export?${params.toString()}`, buildExcelFilename("관리업무-목록"));
  };
  ...
  const loadData = async (page: number = 1) => {
    setIsLoading(true);
    try {
      const result = await getWorkRequestsByClientIdForEmployee(clientId, currentEmployeeId, {
        statusFilter,
        searchType,
  ...
  // 필터링된 데이터의 총 개수
  const filteredTotalCount = searchType === "manager" && searchKeyword ? filteredWorkRequests.length : totalCount;
  ...
  const handleSelectAll = (checked: boolean) => {
    if (checked) {
      const currentDisplay = searchType === "manager" && searchKeyword ? filteredWorkRequests : workRequests;
  ...
  // 상태 변경 확인 모달 열기
  const handleStatusChangeClick = (workRequestId: string, workRequestBrand: string, newStatus: "in_progress" | "completed") => {
    setConfirmModal({
      isOpen: true,
      workRequestId,
      workRequestBrand,
  ...
  const handleConfirmStatusChange = async () => {
    ...
    try {
      const result = await updateWorkRequestStatus(confirmModal.workRequestId, confirmModal.newStatus, currentEmployeeId);

      if (result.success) {
        // 로컬 상태 업데이트
        setWorkRequests((prev) => prev.map((wr) => (wr.id === confirmModal.workRequestId ? { ...wr, status: confirmModal.newStatus! } : wr)));
        setConfirmModal({
          isOpen: false,
          workRequestId: null,
          workRequestBrand: null,
  ...
  const handleDelete = async (workRequestId: string) => {
    ...
    const confirmed = confirm("이 업무를 삭제하시겠습니까?\n금액차감형인 경우 차감된 금액이 복구됩니다.");

    if (!confirmed) return;

    setDeletingRequestId(workRequestId);
    try {
      const response = await fetch(`/api/work-request/${workRequestId}/delete`, {
        method: "POST",
      });
  ...
  const displayWorkRequests = searchType === "manager" && searchKeyword ? filteredWorkRequests : workRequests;
        ...
        <div className={styles.boxInner}>
          ...
          <div className={styles.searchBox}>
            <h2 className={styles.pageSubTitle}>
              <span className={styles.companyName}>{clientName}</span> 업무 내역 <span className={styles.workCount}>({totalCount}건)</span>
            </h2>
            <div className={styles.searchFlex}>
              ...
              <select value={searchType} onChange={(e) => setSearchType(e.target.value as "brand" | "manager")}>
              ...
              />
              <button type="button" className="btn btn_lg primary" onClick={handleSearch}>
                검색
              </button>
            </div>
          </div>
            ...
            <div className={styles.tableWrap}>
              <table>
                ...
                <thead>
                  <tr>
                    <th>
                      <input type="checkbox" id="checkAll" checked={displayWorkRequests.length > 0 && selectedIds.length === displayWorkRequests.length} onChange={(e) => handleSelectAll(e.target.checked)} />
                    </th>
                    <th>번호</th>
                    <th>브랜드명</th>
                    <th>담당자</th>
                ...
                <tbody>
                  {isLoading ? (
                    ...
                    displayWorkRequests.map((wr, index) => {
                      ...
                      const dropdownPosition = isOpen ? getDropdownPosition(wr.id) : { top: 0, left: 0 };
                      const canChangeStatus = wr.employee_id === currentEmployeeId && (wr.status === "approved" || wr.status === "in_progress");

                      return (
                        <tr key={wr.id} onClick={() => handleOpenDetailModal(wr.id)} style={{ cursor: "pointer" }} className={styles.clickableRow}>
                          <td onClick={(e) => e.stopPropagation()}>
                            <input type="checkbox" className="rowCheck" checked={selectedIds.includes(wr.id)} onChange={() => handleToggleSelect(wr.id)} />
                          </td>
                          <td>{filteredTotalCount - (currentPage - 1) * itemsPerPage - index}</td>
                          <td>{wr.brand_name || "-"}</td>
                          <td>{wr.employee_name || "-"}</td>
                          <td>{formatDate(wr.created_at)}</td>
                          <td>{formatDate((wr as any).approved_at)}</td>
                          <td>{formatDate(wr.updated_at)}</td>
                          <td>{formatWorkPeriod(wr.start_date, wr.end_date)}</td>
                          <td className={styles.textOverflow} onClick={(e) => e.stopPropagation()}>
                            <p>{wr.work_content || "-"}</p>
                          </td>
                          <td className={styles.statusCell} onClick={(e) => e.stopPropagation()}>
                            {canChangeStatus ? (
                              <div
                                ...
                                <button id={`status-button-${wr.id}`} type="button" className={`${styles.statusBadge} ${getStatusClass(wr.status)}`} onClick={() => toggleDropdown(wr.id)}>
                                ...
                                {isOpen && (
                                  <div
                                    ...
                                    <ul className={styles.dropdownMenu}>
                                      {wr.status === "approved" && (
                                        <li>
                                          <button type="button" onClick={() => handleStatusChangeClick(wr.id, wr.brand_name || "", "in_progress")}>
                                            작업중
                                          </button>
                                        </li>
                                      )}
                                      {wr.status === "in_progress" && (
                                        <li>
                                          <button type="button" onClick={() => handleStatusChangeClick(wr.id, wr.brand_name || "", "completed")}>
                                            작업완료
                                          </button>
                                        </li>
                                      )}
                              ...
                              <span className={`${styles.statusBadge} ${getStatusClass(wr.status)}`}>{getStatusLabel(wr.status)}</span>
                            )}
                          </td>
                        </tr>
                      );
        ...
        <div
          ...
          <div className={styles.modalContent} onClick={(e) => e.stopPropagation()}>
            ...
            <div className={styles.modalBody}>
              <p>
                다음 업무의 상태를 <strong>{confirmModal.newStatus === "in_progress" ? "작업중" : "작업완료"}</strong>
                으로 변경하시겠습니까?
              </p>
              <div className={styles.modalInfo}>
                ...
                <p className={styles.warningText}>⚠️ 이 작업은 되돌릴 수 없습니다.</p>
              </div>
            </div>
            <div className={styles.modalFooter}>
              <button type="button" className={`${styles.btn} ${styles.normal}`} onClick={handleCancelStatusChange}>
                취소
              </button>
              <button type="button" className={`${styles.btn} ${styles.primary}`} onClick={handleConfirmStatusChange}>
                확인
              </button>
            </div>
          </div>
          ...
          <div className={styles.detailModalInner} onClick={(e) => e.stopPropagation()}>
            ...
            <div className={`${styles.detailModalBody} ${styles.scroll}`}>
              {detailModal.isLoading ? (
                  ...
                  {detailModal.workRequest.managed_client && (
                    <div className={styles.tableItem}>
                        ...
                        <>
                          <ul className={styles.tableRow}>
                            <li className={styles.rowGroup}>
                              ...
                              <div className={`${styles.tableData} ${styles.fontB}`}>
                                {(() => {
                                  ...
                                  if (detailModal.workRequest.status === "approved" || detailModal.workRequest.status === "in_progress" || detailModal.workRequest.status === "completed") {
                                    ...
                                    if (remaining !== null && remaining !== undefined && deducted !== null && deducted !== undefined) {
                                    ...
                                    return detailModal.workRequest.managed_client?.totalAmount ? detailModal.workRequest.managed_client.totalAmount.toLocaleString("ko-KR") : "-";
                                  }
                            ...
                            <li className={styles.rowGroup}>
                              ...
                              <div className={`${styles.tableData} ${styles.fontB}`}>{detailModal.workRequest.approval_deducted_amount !== null && detailModal.workRequest.approval_deducted_amount !== undefined ? detailModal.workRequest.approval_deducted_amount.toLocaleString() : "-"}</div>
                            </li>
                          </ul>
                          <ul className={styles.tableRow}>
                            <li className={styles.rowGroup}>
                              <div className={styles.tableHead}>잔여 금액</div>
                              <div className={`${styles.tableData} ${styles.fontB}`}>{detailModal.workRequest.approval_remaining_amount !== null && detailModal.workRequest.approval_remaining_amount !== undefined ? detailModal.workRequest.approval_remaining_amount.toLocaleString() : "-"}</div>
                            </li>
                          </ul>
                        </>
                      )}
                      ...
                      {detailModal.workRequest.managed_client.productType1 === "maintenance" && (
                        <ul className={styles.tableRow}>
                          <li className={styles.rowGroup}>
                            <div className={styles.tableHead}>세부 내용 (잔여)</div>
                            <div className={styles.tableData}>
                              <ul className={styles.detailList}>
                                <li>
                                  영역 텍스트 수정
                                  <span>
                                    <b className={styles.fontB}>{detailModal.workRequest.approval_text_edit_count !== null && detailModal.workRequest.approval_text_edit_count !== undefined ? detailModal.workRequest.approval_text_edit_count : "-"}</b>회
                                  </span>
                                </li>
                                <li>
                                  코딩 수정
                                  <span>
                                    <b className={styles.fontB}>{detailModal.workRequest.approval_coding_edit_count !== null && detailModal.workRequest.approval_coding_edit_count !== undefined ? detailModal.workRequest.approval_coding_edit_count : "-"}</b>회
                                  </span>
                                </li>
                                <li>
                                  기존 결과물 이미지 수정
                                  <span>
                                    <b className={styles.fontB}>{detailModal.workRequest.approval_image_edit_count !== null && detailModal.workRequest.approval_image_edit_count !== undefined ? detailModal.workRequest.approval_image_edit_count : "-"}</b>회
                                  </span>
                                </li>
                                <li>
                                  팝업 디자인
                                  <span>
                                    <b className={styles.fontB}>{detailModal.workRequest.approval_popup_design_count !== null && detailModal.workRequest.approval_popup_design_count !== undefined ? detailModal.workRequest.approval_popup_design_count : "-"}</b>회
                                  </span>
                                </li>
                                {detailModal.workRequest.managed_client.productType2 === "premium" && (
                                  <li>
                                    배너 디자인
                                    <span>
                                      <b className={styles.fontB}>{detailModal.workRequest.approval_banner_design_count !== null && detailModal.workRequest.approval_banner_design_count !== undefined ? detailModal.workRequest.approval_banner_design_count : "-"}</b>회
                                    </span>
                                  </li>
                                )}
                              </ul>
                      ...
                      <ul className={styles.tableRow}>
                        <li className={styles.rowGroup}>
                          ...
                          <div className={styles.tableData}>{detailModal.workRequest.managed_client.startDate && detailModal.workRequest.managed_client.endDate ? `${formatDate(detailModal.workRequest.managed_client.startDate)} ~ ${formatDate(detailModal.workRequest.managed_client.endDate)}` : "-"}</div>
                        </li>
                      </ul>
                      <ul className={styles.tableRow}>
                        <li className={styles.rowGroup}>
                          ...
                          <div className={styles.tableData}>
                            <span className={detailModal.workRequest.managed_client.status === "ongoing" ? styles.statusOngoing : detailModal.workRequest.managed_client.status === "wait" ? styles.statusWait : detailModal.workRequest.managed_client.status === "end" ? styles.statusEnd : detailModal.workRequest.managed_client.status === "unpaid" ? styles.statusUnpaid : ""}>
                              {detailModal.workRequest.managed_client.status === "ongoing" ? "진행" : detailModal.workRequest.managed_client.status === "wait" ? "대기" : detailModal.workRequest.managed_client.status === "end" ? "종료" : detailModal.workRequest.managed_client.status === "unpaid" ? "미납" : detailModal.workRequest.managed_client.status}
                            </span>
                          </div>
                        </li>
                      </ul>
                  ...
                  <div className={styles.tableItem}>
                    ...
                    <ul className={styles.tableRow}>
                      <li className={styles.rowGroup}>
                        ...
                        <div className={styles.tableData}>{detailModal.workRequest.brand_name || "-"}</div>
                      </li>
                    </ul>
                    <ul className={styles.tableRow}>
                      <li className={styles.rowGroup}>
                        <div className={styles.tableHead}>담당자</div>
                        <div className={styles.tableData}>{detailModal.workRequest.employee_name || "-"}</div>
                      </li>
                    </ul>
                    <ul className={styles.tableRow}>
                      <li className={styles.rowGroup}>
                        <div className={styles.tableHead}>요청일</div>
                        <div className={styles.tableData}>{formatDate(detailModal.workRequest.created_at)}</div>
                      </li>
                      <li className={styles.rowGroup}>
                        <div className={styles.tableHead}>승인일</div>
                        <div className={styles.tableData}>{formatDate((detailModal.workRequest as any).approved_at)}</div>
                      </li>
                    </ul>
                    <ul className={styles.tableRow}>
                      <li className={styles.rowGroup}>
                        <div className={styles.tableHead}>상태변경일</div>
                        <div className={styles.tableData}>{formatDate(detailModal.workRequest.updated_at)}</div>
                      </li>
                      <li className={styles.rowGroup}>
                        <div className={styles.tableHead}>작업기간</div>
                        <div className={styles.tableData}>{formatWorkPeriod(detailModal.workRequest.start_date, detailModal.workRequest.end_date)}</div>
                      </li>
                    </ul>
                      ...
                      <>
                        <ul className={styles.tableRow}>
                          <li className={styles.rowGroup}>
                            ...
                            <div className={styles.tableData}>{(detailModal.workRequest as any).cost ? Number((detailModal.workRequest as any).cost).toLocaleString() : "-"}</div>
                          </li>
                          <li className={styles.rowGroup}>
                        ...
                        <ul className={styles.tableRow}>
                          <li className={styles.rowGroup}>
                            ...
                            <div className={styles.tableData}>
                              <span className={getStatusClass(detailModal.workRequest.status)}>{getStatusLabel(detailModal.workRequest.status)}</span>
                            </div>
                          </li>
                        </ul>
                        <ul className={styles.tableRow}>
                          <li className={styles.rowGroup}>
                            ...
                            <div className={styles.tableData}>{detailModal.workRequest.work_content || "-"}</div>
                          </li>
                        </ul>
                      </>
                    )}
                      ...
                      <>
                        ...
                        <ul className={styles.tableRow}>
                          ...
                          <li className={styles.rowGroup}>
                            ...
                            <div className={styles.tableData}>{(detailModal.workRequest as any).count || "-"}</div>
                          </li>
                        </ul>
                      </>
                    )}
                ...
                <div style={{ textAlign: "center", padding: "40px 0" }}>업무 정보를 불러올 수 없습니다.</div>
              )}
            </div>
