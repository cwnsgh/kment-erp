...
export function ClientWorkList({ initialWorkRequests, initialTotalCount, clientName = "" }: ClientWorkListProps) {
  ...
  const loadData = async (page: number, filter: typeof statusFilter, limit: number) => {
    startTransition(async () => {
      try {
        const response = await fetch(`/api/client/work-requests?statusFilter=${filter}&page=${page}&limit=${limit}`);
        if (response.ok) {
  ...
  // 작업기간 포맷 (모달용)
  const formatWorkPeriodForModal = (startDate: string | null | undefined, endDate: string | null | undefined) => {
  ...
  const filteredRequests = workRequests.filter((r) => {
    // deleted 상태는 제외
    if (r.status === "deleted") return false;
    // approved, in_progress, completed만 표시
    return r.status === "approved" || r.status === "in_progress" || r.status === "completed";
  });
        ...
        <div className={styles.detailModalOverlay} onClick={handleCloseDetailModal}>
          <div className={styles.detailModalInner} onClick={(e) => e.stopPropagation()}>
                    ...
                    <div className={styles.tableItem}>
                      ...
                      <ul className={styles.tableRow}>
                        <li className={styles.rowGroup}>
                          ...
                          <div className={`${styles.tableData} ${styles.fontB}`}>{detailModal.workRequest.managed_client.productType1 === "deduct" ? "금액차감형" : "유지보수형"}</div>
                        </li>
                        <li className={styles.rowGroup}>
                      ...
                      {detailModal.workRequest.managed_client.productType1 === "maintenance" && (
                        <ul className={styles.tableRow}>
                          <li className={styles.rowGroup}>
                            <div className={styles.tableHead}>세부 네영 (잔여)</div>
                      ...
                      <ul className={styles.tableRow}>
                        <li className={styles.rowGroup}>
                          ...
                          <div className={styles.tableData}>{detailModal.workRequest.managed_client.startDate && detailModal.workRequest.managed_client.endDate ? `${formatWorkPeriodForModal(detailModal.workRequest.managed_client.startDate, detailModal.workRequest.managed_client.endDate)}` : "-"}</div>
                        </li>
                      </ul>
                      <ul className={styles.tableRow}>
                        <li className={styles.rowGroup}>
                  ...
                  <div className={styles.tableItem}>
                    ...
                    <ul className={styles.tableRow}>
                      ...
                      <li className={styles.rowGroup}>
                        ...
                        <div className={styles.tableData}>{formatWorkPeriodForModal(detailModal.workRequest.start_date, detailModal.workRequest.end_date)}</div>
                      </li>
                    </ul>
                      ...
                      <>
                        <ul className={styles.tableRow}>
                          <li className={styles.rowGroup}>
                            ...
                            <div className={styles.tableData}>{detailModal.workRequest.cost ? Number(detailModal.workRequest.cost).toLocaleString() : "-"}</div>
                          </li>
                          <li className={styles.rowGroup}>
                            <div className={styles.tableHead}>승인날짜</div>
                            <div className={`${styles.tableData} ${styles.column}`}>
                              {detailModal.workRequest.approved_at ? formatDateForModal(detailModal.workRequest.approved_at) : "-"}
                      ...
                      <>
                        ...
                        <ul className={styles.tableRow}>
                          ...
                          <li className={styles.rowGroup}>
                            ...
                            <div className={styles.tableData}>{detailModal.workRequest.count || "-"}</div>
                          </li>
                        </ul>
                      </>
                    )}
            ...
            <div className={styles.detailModalFooter}>
              <button type="button" className={`${styles.btn} ${styles.btnLg} ${styles.normal} btn btn_lg normal`} onClick={handleCloseDetailModal}>
                닫기
              </button>
            </div>
          </div>
